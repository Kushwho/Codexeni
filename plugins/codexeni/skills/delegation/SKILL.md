...
2. Call `delegate_discover` once per session. Pick a harness whose `installed` is `true` and `authStatus` is `authenticated`, and an exact model from that harness's listed `models`. Codex reports its maintained compatible model list with `modelSource: "static"` because its CLI cannot enumerate the authenticated account's entitlements; the Codex CLI remains the final authority if an account cannot use a listed model. For a Codex task, always pass that chosen name as `delegate_start.model`; the bridge rejects an omitted model.
